...
const form = useAuthAccountForm({ role: 'user', scene: 'register' })
  ...
  <PhoneShell>
    <div class="auth-page">
      ...
      <main class="auth-card">
        ...
        <!-- 账号注册协议（默认勾选） -->
        <div v-if="showAgreement" class="agree-line">
          <van-checkbox v-model="agree" shape="square" icon-size="15px" class="agree-check">
            <span class="agree-text">
              {{ t('register.agreeText') }}
              <span class="agree-link" @click.stop="$router.push('/agreement/register')">{{ t('register.agreeTerms') }}</span>
              {{ t('register.and') }}
              <span class="agree-link" @click.stop="$router.push('/agreement/privacy')">{{ t('register.agreePrivacy') }}</span>
            </span>
          </van-checkbox>
        </div>
